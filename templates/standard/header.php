<?php
/**
 * Template view: standard (type_id=1) — tin thường, left-aligned, không cover.
 * Port 1:1 từ frontsite views/components/post-header-standard.php.
 *
 * Scope: chỉ có $vm (HeaderViewModel) + $adapter (HeaderViewAdapter). KHÔNG host global.
 * Giữ ĐÚNG escape gốc: href/img src để RAW (qua adapter), text qua htmlspecialchars.
 *
        ...
        <div class="td-post-header__topic">
            <a href="<?= $adapter->urlFor('nhom-chu-de/' . $vm->topicSlug) ?>"
               title="Nhóm chủ đề: <?= htmlspecialchars($vm->topicName ?? '') ?>">
                <img src="<?= $adapter->imageUrl($vm->topicAvatarSource, $adapter::VARIANT_TOPIC) ?>"
